...
class ValidationCacheTests(unittest.TestCase):
    def test_kfold_trains_once_per_subject_fold_without_patient_overlap(self):
        files = [f"f{i}.edf" for i in range(6)]
        file_to_subject = {
            name: f"p{idx // 2}" for idx, name in enumerate(files)
        }
        features = {
        ...
        for train_ids, val_ids in calls:
            ...
            train_subjects = {
                file_to_subject[files[index]] for index in train_ids
            }
            val_subjects = {
                file_to_subject[files[index]] for index in val_ids
            }
